...
function TfrmMain.ShellExecuteAndWait(FileName: string; Params: string): bool;
var
...
begin
  ...
  sSlicerRunArgs := sSlicerRunArgs + ' --load "' + sInstalledPath + '\bin\slic3r\config.ini"';
  sSlicerRunArgs := sSlicerRunArgs + ' "' + sSiteForgePath + '\stl-backups\' + sName + '.stl"';

  sSlicerExecutable := '"' + sInstalledPath + '\bin\slic3r\Slic3r-debug-console.exe"';
